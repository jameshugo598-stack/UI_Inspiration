...
document.addEventListener('DOMContentLoaded', () => {
    ...
    templates.forEach(template => {
        ...
        card.className = 'group bg-zinc-900/50 border border-white/5 rounded-2xl p-3 hover:border-emerald-500/30 transition-all hover:bg-zinc-900 duration-500 cursor-pointer';
        card.innerHTML = `
            <div class="aspect-[16/10] bg-black rounded-xl mb-4 overflow-hidden relative">
                <iframe src="${template.file}" class="w-[200%] h-[200%] scale-0.5 origin-top-left border-none pointer-events-none opacity-60 group-hover:opacity-100 transition-all duration-500 filter contrast(1.1) brightness(1.1)"></iframe>
            ...
            <div class="px-2 pb-2">
                ...
                <div class="flex items-center gap-2">
                    <button class="flex-1 bg-white text-black py-2 rounded-lg text-xs font-bold hover:bg-emerald-400 transition-colors preview-btn" data-file="${template.file}" data-name="${template.name}">Preview View</button>
                    ...
                    <a href="${template.file}" download onclick="event.stopPropagation()" class="w-10 h-8 flex items-center justify-center border border-white/10 rounded-lg hover:bg-white/5 transition-colors">
